...
#######################
def clean_text(text, accept_words, remove_stop_words = False, stemming = False, lemmatization = False):
    text = str(text)
    ...
    #text = re.sub(r"[,;!.:%#*&+=\")(_~/<>{}]?", '', text)  ## NO REMOVE ' and - because there are ' mark in n't and - mark in uh-
    #text = text.replace('[', '')
    #text = text.replace(']', '')
    ...
    ## Lemmatization and delete the words which just have 1 character 
    wordnet_lemmatizer = WordNetLemmatizer()
    if lemmatization:
        text = [wordnet_lemmatizer.lemmatize(w, pos="v") for w in text if (len(w) >= 2  or w in accept_words)] #and w not in except_words
    else:
        text = [w for w in text if (len(w) >= 2 or w in accept_words)] #and w not in except_words
...
def tranformid(set_idx):
    numberList = []
    for idx in set_idx:
        numberList.append(transform(idx))
    return numberList

if __name__ == '__main__':
    
    ##### CHOOSE hyperparameter for preprocessing
    ...
    ##########
    path = "..."
    all_files = glob.glob(path + "/swda/**/*.csv")  
    li = []
    concatenated_df  = pd.concat(li, axis=0, ignore_index=True)
    
    concatenated_df['text'] = concatenated_df['text'].apply(lambda x: clean_text(x, remove_stop_words = remove_stop_words, stemming = stemming, lemmatization = lemmatization))


    concatenated_df['dialogue_act'] = concatenated_df['act_tag'].apply(damsl_act_tag) 
    concatenated_df = concatenated_df[concatenated_df['dialogue_act'] != '+'] 
    
    concatenated_df['doc'] = concatenated_df['swda_filename']
    
    concatenated_df['conversation_no'] = concatenated_df['conversation_no'].astype(int)
    #concatenated_df['conversation_no'] = concatenated_df['conversation_no'].astype('category')
    #concatenated_df['dialogue_num'] = concatenated_df['conversation_no'].cat.codes
    
    concatenated_df['dialogue_act'] = concatenated_df['dialogue_act'].astype('category')
    concatenated_df['label'] = concatenated_df['dialogue_act'].cat.codes
    act_label = dict(enumerate(concatenated_df['dialogue_act'].cat.categories))
    
    data_frame = concatenated_df[['doc','conversation_no','text','dialogue_act','label']]
    
    ##### Separate data index to train index, valid index and test index
    ...
    train_idx = tranformid(train_set_idx)
    valid_idx = tranformid(valid_set_idx)   
    test_idx = tranformid(test_set_idx) 
    ##### Separate data to train data, valid data, test_data
    
    swda_train = data_frame.loc[data_frame['conversation_no'].isin(train_idx)]
    swda_test = data_frame.loc[data_frame['conversation_no'].isin(test_idx)]
    swda_valid = data_frame.loc[data_frame['conversation_no'].isin(valid_idx)]
    swda = data_frame.loc[data_frame['conversation_no'].isin(train_idx+valid_idx+test_idx)]
    
    ##### save data
    swda_train.to_pickle(path + '/data/swda_train.pkl')

    swda_test.to_pickle(path + '/data/swda_test.pkl')

    swda_valid.to_pickle(path + '/data/swda_valid.pkl')
    
    swda.to_pickle(path + '/data/swda.pkl')
